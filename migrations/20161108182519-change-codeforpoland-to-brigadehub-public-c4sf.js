...
exports.up = function (db, cb) {
  return connect(function (err, disconnect) {
    ...
    Brigade.findOne({}, function (err, brigade) {
      if (err) return disconnect(cb, err)
      if (brigade.theme.slug === 'codeforpoland') {
        brigade.theme.slug = 'brigadehub-public-c4sf'
...
exports.down = function (db, cb) {
  return connect(function (err, disconnect) {
    ...
    Brigade.findOne({}, function (err, brigade) {
      if (err) return disconnect(cb, err)
      if (brigade.theme.slug === 'brigadehub-public-c4sf') {
        brigade.theme.slug = 'codeforpoland'
        return brigade.save(function (err, b) {
          disconnect(cb, err)
        })
      }
